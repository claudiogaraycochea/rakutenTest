...
const getUserProjects = (uid) => {
	...
	return (dispatch) => {
		axios.get(`${API_URL}/users/${uid}/projects`, params)
			.then((response) => {
				if (response.data.data.role !== 'sponsor') {
					axios.get(`${API_URL}/users/${uid}/projects/assignee`, params)
						.then((resp) => {
							response.data.data.projects = resp.data.data;
							dispatch({
...
const getUserProjectsAssignee = (data) => {
	const { uid } = data;
	const params = { uid };
	return (dispatch) => {
		axios.get(`${API_URL}/users/${uid}/projects/assignee`, params)
...
export {
	userLogin, getUserProjects, signUp, userUpdate, getUser, getUserFiles, getUserProjectsAssignee,
};
